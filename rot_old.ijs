NB. Rotation
NB.
NB. lartg_old      Generates a plane rotation of a 2-vector
NB. rot_old        Applies a plane rotation[s] to a 2-vector[s]
NB. rotga_old      Adv. to make verb to get and apply rotation
NB. rotsclx    Update array by rotations and scalings
NB.            accumulated
NB.
NB. testlartg_old  Test lartg_old by vectors given
NB. testrot_old    Test rotation algorithms by predefined matrix
NB.
...
morim_old=: >./"1@:|@:+.    NB. monad: max of real and imaginary parts' modules, max(|Re(y)|,|Im(y)|)
sgn_old=: (*!.0)`1:@.(0&=)  NB. monad: (if y<0 then -1 else 1 endif), for reals equiv. to: (0&(<:->))
...
NB.
NB. Description:
NB.   Generate matrix to test lartg_old
NB.
NB. Syntax:
NB.   m=. rottestmat y
NB. where
...
NB. - numbers are selected to force lartg_old to go through all
...
lartg_old=: 3 : 0
  'f g'=. y
  'scalef scaleg'=. scalefg=. morim_old y
  ...
  elseif. do.
    ...
    'c s r'=. lartg_oldc1 y * iof { ROTSCL20
    r=. r * iof { ROTSCL21
...
NB.
NB. Syntax:
NB.   vapp=. vrota rotga_old
NB. where
NB.   vrota   - dyad to apply rotation; is called as:
NB.               subAupd=. cs vrota subA
NB.             and is any of:
NB.               rot_old      NB. apply rotation to rows
NB.               rot_old&.|:  NB. apply rotation to columns
NB.   vapp    - monad to generate and apply rotation; is
NB.             called as:
NB.               'Aupd cs'=. vapp A ; iossubA ; iosfg
NB.   cs      - 2-vector (c,s), curtailed output of lartg_old,
...
NB.   to avoid lartg_old roundoff errors

rotga_old=: 1 : 0
  'A iossubA iosfg'=. y
  subA=. iossubA { A
  csr=. lartg_old iosfg { subA
...
NB.             (A*dA) otherwise
NB.   m       - multiplier to scale either row (rotscll_old) or
NB.             column (rotsclu_old)
NB.   io      - IO either row (rotscll_old) or column (rotsclu_old)
NB.             to scale
NB.   cs      - 2-vector (c,s), curtailed output of lartg_old,
NB.             defines rotation matrix
NB.   iof,iog - IOS either rows (rotscll_old) or columns
NB.             (rotsclu_old) which contain 2-vectors (f,g) to
NB.             rotate, iof≠iog
NB.
NB. TODO:
...
rotscll_old=: (4 : 0)
  ...
  while. i < # y do.                 NB. traverse dA rows down
    ...
    if. 0 0 -: iofg do.
      ...
      x=. iofg (cs&(rot_old&.|:)) upd x  NB. do rotation
    end.
    i=. >: i
  end.
  x
...
rotsclu_old=: (4 : 0)
  ...
  while. i < # y do.                    NB. traverse dA rows down
    ...
    if. 0 0 -: iofg do.
      ...
      x=. (< a: ; iofg) (cs&rot_old) upd x  NB. do rotation
    end.
    i=. >: i
  end.
  x
...
NB.
NB. Description:
NB.   Test lartg_old by vectors given
NB.
NB. Syntax:
NB.   testlartg_old A
NB. where
...
NB.        R[i]      - approximation computed by lartg_old,
...
testlartg_old=: 3 : 0
  ...
  algo1=. 3 : 'if. 0 = {: y do. (sgn_old,0:,]) {. y elseif. 0 = {. y do. (0,(sgn_old@+),|) {: y elseif. do. try. ((d*|f),(sgn_oldf*(+g)*d),(sgn_oldf % d)) [ sgn_oldf=. sgn_old f [ d=. % %: (+/) soris ''f g''=. y catch. 3 # _. end. end.'
  ...
  ('lartg_old"1' tmonad (]`]`(_."_)`(_."_)`(vberr f.))) y  NB. fix vberr to allow trans-locale calling
...
testrot_old=: EMPTY [ testlartg_old@rottestmat
