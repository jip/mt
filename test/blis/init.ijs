...
NB.
NB. Description:
NB.   Nilad to find BLIS dynamic library and return the FQFN
...
findlib=: 3 : 0
  select. UNAME
    case. 'Win' do.
      paths=. < jpath '~'
      names=. 'libblis.dll' ; 'libflame.dll'
    case. 'Linux' ; 'OpenBSD' ; 'FreeBSD' do.
      h=. getenv 'HOME'
      paths=. (h ,L:0 '/lib' ; '/blis/lib' ; '/flame/lib') , '/usr/local/lib' ; '/usr/lib' ; '/opt/blis/lib' ; '/opt/flame/lib'
      arch=. '' ;~ IF64 { '32' ,: '64'
      paths=. , ;L:_1 { paths ,&< arch
      names=. 'libblis.so' ; 'libflame.so'
    case. 'Android' do.
